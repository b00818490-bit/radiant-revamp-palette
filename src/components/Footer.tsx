...
import { Instagram, Facebook, Youtube, Mail, ShieldCheck, Leaf, Sparkles } from "lucide-react";
        ...
        <div className="grid gap-12 md:grid-cols-6">
          <div className="md:col-span-2">
            ...
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-ivory/60">
              Clean color. Serious skincare. Dermatologist-tested formulas designed and made
              responsibly.
            </p>
            ...
            <div className="mt-8 space-y-2 text-xs text-ivory/60">
              ...
              <p>
                <a href="mailto:hello@greyon.co" className="hover:text-ivory">hello@greyon.co</a>
              </p>
          ...
          {columns.map((col) => (
            <div key={col.title}>
              <p className="text-[11px] uppercase tracking-[0.28em] text-ivory">{col.title}</p>
              <ul className="mt-5 space-y-3 text-sm text-ivory/65">
                {col.links.map((l) =>
                  "to" in l && l.to ? (
                    <li key={l.label}>
                      {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                      <Link to={l.to as any} params={l.params as any} className="transition-colors hover:text-gold">
                        {l.label}
                      </Link>
                    </li>
                  ) : (
                    <li key={l.label}>
                      <a href={(l as { href: string }).href} className="transition-colors hover:text-gold">
                        {l.label}
                      </a>
                    </li>
                  ),
